    ...
    <section id="education" className="relative py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <div className="lg:col-span-6 flex flex-col gap-6">
          <div>
            <h2 className="font-display font-bold text-3xl sm:text-4xl text-slate-900 dark:text-brand-warm-gray tracking-tight flex items-center gap-3">
              <GraduationCap className="w-8 h-8 text-brand-burgundy" />
              <span>Education</span>
            </h2>
              ...
              >
                <div className="flex items-start justify-between gap-4 mb-2">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-lg bg-brand-burgundy/10 border border-brand-burgundy/20 flex items-center justify-center text-brand-burgundy">
                  ...
                  {edu.status && (
                    <span className="px-2.5 py-1 rounded-full text-[10px] font-mono font-semibold bg-brand-burgundy/10 text-brand-burgundy border border-brand-burgundy/30 shrink-0">
                      {edu.status}
                    </span>
                  )}
                </div>
        ...
        <div className="lg:col-span-6 flex flex-col gap-6">
          <div>
            <h2 className="font-display font-bold text-3xl sm:text-4xl text-slate-900 dark:text-brand-warm-gray tracking-tight flex items-center gap-3">
              <Languages className="w-8 h-8 text-brand-slate" />
            ...
            {languages.map((lang, idx) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  <span className="font-display font-bold text-base text-slate-900 dark:text-brand-warm-gray">
                    {lang.language}
                  </span>
                  {lang.nativeName && (
                ...
                <div className="flex items-center justify-between pt-2 border-t border-slate-200/60 dark:border-white/5 text-xs">
                  ...
                  <span className="font-mono text-[11px] px-2 py-0.5 rounded bg-slate-100 dark:bg-white/5 text-brand-slate border border-slate-200 dark:border-white/5">
                    {lang.levelCode}
                  </span>
                </div>
              </motion.div>
